...
def inject_css() -> None:
    ...
    st.markdown("""
    <style>
    .stApp { background-color: #FFF8FB; }

    .main-title {
        text-align: center; font-size: 2.4rem; font-weight: 800;
        color: #D63384; margin-bottom: 0.2rem; letter-spacing: 0.05em;
    }
    .main-subtitle {
    ...
    .visitor-badge {
        text-align: center; color: #F06292;
        font-size: 0.85rem; margin-bottom: 1.2rem;
    }

    .announcement-card {
        background: linear-gradient(135deg, #FFF0F5, #FCE4EC);
        border-left: 5px solid #F06292; border-radius: 16px;
        padding: 1rem 1.25rem; margin-bottom: 0.75rem;
        box-shadow: 0 2px 8px rgba(240,98,146,0.12);
    }
    .announcement-title { font-size:1.05rem; font-weight:700; color:#C2185B; margin-bottom:0.3rem; }
    .announcement-content { color:#555; font-size:0.92rem; line-height:1.6; white-space:pre-wrap; }
    .announcement-meta { font-size:0.78rem; color:#E91E63; margin-top:0.4rem; opacity:0.7; }

    .message-card {
        background: #FFFFFF; border-radius: 18px;
        padding: 1rem 1.25rem; margin-bottom: 0.85rem;
        box-shadow: 0 2px 10px rgba(211,47,47,0.07);
        border: 1px solid #FCE4EC; transition: box-shadow 0.2s;
    }
    .message-card:hover { box-shadow: 0 4px 16px rgba(240,98,146,0.15); }
    .message-name { font-weight:700; color:#D81B60; font-size:0.95rem; }
    .message-content { color:#333; font-size:0.92rem; margin:0.35rem 0; line-height:1.65; white-space:pre-wrap; word-break:break-word; }
    .message-time { font-size:0.75rem; color:#BDBDBD; }

    .pager-info { text-align:center; color:#F06292; font-size:0.88rem; margin:0.5rem 0; }
    .section-header { font-size:1.1rem; font-weight:700; color:#AD1457; margin:1rem 0 0.5rem; }
    .empty-hint { text-align:center; color:#F48FB1; font-size:0.95rem; padding:1.5rem; }

    div[data-testid="stTextInput"] input,
    div[data-testid="stTextArea"] textarea {
        border-radius: 12px; border: 1.5px solid #F8BBD9; background: #FFF9FB;
    }
    div[data-testid="stTextInput"] input:focus,
    div[data-testid="stTextArea"] textarea:focus {
        border-color: #F06292; box-shadow: 0 0 0 2px rgba(240,98,146,0.2);
    }
    div[data-testid="stButton"] > button { border-radius: 20px; font-weight: 600; }
    section[data-testid="stSidebar"] { background: #FFF0F5; }
    </style>
...
def render_post_form() -> None:
    ...
    with st.form(key="post_form", clear_on_submit=True):
        name = st.text_input("暱稱 🐰", placeholder="請輸入你的暱稱", max_chars=30)
        ...
        submitted = st.form_submit_button("送出留言 ✨", use_container_width=True, type="primary")

    if submitted:
        _handle_post(name, content)


def _handle_post(name: str, content: str) -> None:
    """處理留言送出的驗證與寫入。"""
    ...
    if is_duplicate_message(name, content, seconds=60):
        st.warning("你剛才已經留過相同的留言了 🐰")
        return

    if create_message(name, content):
...
def render_messages() -> None:
    ...
    if not messages:
        st.markdown('<div class="empty-hint">還沒有留言，來當第一個吧 🌸</div>', unsafe_allow_html=True)
        return

    for msg in messages:
        st.markdown(f"""
        <div class="message-card">
            <div class="message-name">🐰 {msg.get("name", "匿名")}</div>
...
def _render_admin_messages() -> None:
    ...
    for msg in messages:
        ...
        <div class="message-card" style="{fade}">
            <div class="message-name">🐰 {msg.get("name","匿名")}
...
def main() -> None:
    ...
    st.markdown(
        f'<div class="visitor-badge">👀 累計訪客：{visitor_count} 人次</div>',
        unsafe_allow_html=True,
    )
